...
$(function () {
    // setup garden
	$loveHeart = $("#loveHeart");
	// var offsetX = $(".post-body").width() / 2;
	// var offsetY = $(".post-body").height() / 2 - 55;
    $garden = $("#garden");
    gardenCanvas = $garden[0];
	gardenCanvas.width = $(".post-body").width() + 40;
...
function typeCode() {
	$ele = $("#code"), str = $ele.html(), progress = 0;
	$ele.html('');
	$("#code").css("display","");
	var timer = setInterval(function() {
		var current = str.substr(progress, 1);
		...
		if (progress >= str.length) {
			clearInterval(timer);
			$(document).snowfall({image :"/images/rose.png", minSize: 10, maxSize:32, flakeCount: 100, maxSpeed: 3});
		}
	}, 175);
}
...
function showMessages2() {
	//adjustWordsPosition();
	$('#messages2').fadeIn(1500, function() {
		showLoveU();
	});
}
